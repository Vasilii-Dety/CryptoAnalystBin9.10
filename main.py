...
# ═══════════════════════════════════════════════════════════════════════
def _safe_load_markets(force_reload=False):
    """v7.3.9.4: load_markets с обработкой 418/Network в бесконечном retry.

    force_reload: при старте всегда False (markets грузятся первый раз,
    reload не нужен). При hourly refresh — True, чтобы подхватить новые
    листинги. Один reload раз в час по уже работающему боту с чистого IP
    безопасен; опасен был reload в retry-цикле НА СТАРТЕ при бане (см. ниже).

    Два ключевых изменения против эскалации бана (как в рабочем памп-боте):
    1. load_markets() БЕЗ reload=True. reload=True форсил новый REST-запрос
       на КАЖДОЙ retry-попытке → 12 ударов в забаненный IP (см. логи #12) →
       Binance наращивал срок бана. Без reload: если markets уже загружены,
       ccxt берёт из кэша и НЕ бьёт по REST повторно.
    2. При активном бане спим ПОЛНУЮ длительность бана + запас (не порциями
       по 60с). Порционный сон будил код раньше реального снятия бана, он
       преждевременно стучался в REST и продлевал бан. Heartbeat шлём ДО сна.
    """
    attempt = 0
    while True:
        attempt += 1

        # Проверяем флаг бана. Если активен — спим до конца бана + запас 15с,
        # одним сном целиком (не будим код раньше времени).
        with _IP_BAN_LOCK:
            ban_until_now = IP_BAN_UNTIL
        if ban_until_now > time.time():
            wait = ban_until_now - time.time() + 15
            logging.warning(
                f"🔄 _safe_load_markets #{attempt}: ban активен до "
                f"{datetime.fromtimestamp(ban_until_now).strftime('%H:%M:%S')}, "
                f"sleep {wait:.0f}s (ждём ПОЛНОСТЬЮ, без повторных REST-ударов)"
            )
            time.sleep(wait)
            continue
        ...
        try:
            ...
            try:
                # v7.3.9.4: reload только если запрошен (hourly refresh) И это
                # первая попытка. После любого 418 reload сбрасывается, чтобы
                # retry НЕ форсили повторный REST в потенциально горячий IP.
                exchange.load_markets(reload=force_reload)
            finally:
            ...
            ban_ts = _parse_banned_until_ms(err_str)
            if ban_ts:
                force_reload = False  # v7.3.9.4: после бана не форсим reload на retry
                set_ip_ban_until(ban_ts)
                wait = max(ban_ts - time.time(), 0) + 15
                logging.warning(
                    f"🚫 _safe_load_markets #{attempt}: 418 until "
                    f"{datetime.fromtimestamp(ban_ts).strftime('%H:%M:%S')}, "
                    f"sleep {wait:.0f}s (ПОЛНЫЙ срок бана, без досрочных ударов)"
                )
                time.sleep(wait)  # v7.3.9.4: спим весь срок бана целиком
                continue
            if '418' in err_str or 'banned' in err_str.lower() or "teapot" in err_str.lower():
                force_reload = False  # v7.3.9.4: после 418 не форсим reload
                # 418 без распарсенного времени — консервативная пауза 5 минут,
                # чтобы дать IP остыть и не провоцировать эскалацию.
                set_ip_ban_until(time.time() + 300)
                logging.warning(f"🚫 _safe_load_markets #{attempt}: 418 (no ts), sleep 300s")
                time.sleep(300)
                continue
